...
fetch('https://youtube.googleapis.com/youtube/v3/videos?part=snippet%2CcontentDetails%2Cstatistics%2Cplayer&id='+video_id+'&key=AIzaSyCt-7BNhBifrvrNviItFif36rrq8eFWacY')
.then((res)=>{
    return res.json();
}).then((data)=>{
    ...
    const iframeHTML = video_details.player.embedHtml;

    document.getElementById('video_title').innerText = title;

    document.getElementById('channel_name').innerText = channel_Name;

    document.getElementById('video_discription').innerText = description;

    document.getElementById('video_views').innerText = view_count ;

    document.getElementById('video_likes').innerText = like_count ;

    document.getElementById('player_container').src="https://www.youtube.com/embed/"+video_id;

    getsuggestion();
});

function  getsuggestion(){
    fetch('https://youtube.googleapis.com/youtube/v3/search?part=snippet&maxResults=15&relatedToVideoId='+video_id+'&type=video&key=AIzaSyCt-7BNhBifrvrNviItFif36rrq8eFWacY')
    .then((res)=>{
        return res.json();
    }).then((data)=>{
        console.log(data);
        ...
        for (let i = 0; i < suggestion_list.length; i++) {
            ...
            h5_elem.classList.add('class-title');
            h5_elem.innerText = suggestion_video_title;

            const anchor_elem = document.createElement('a')
            anchor_elem.href = `details.html?id=${suggestion_video_id}`
            ...
            suggestion_container.appendChild(card_div_elem);
