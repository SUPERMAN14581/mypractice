...
const search_quary = params.get('q');
console.log(search_quary);

fetch('https://youtube.googleapis.com/youtube/v3/search?part=snippet&maxResults=25&q='+search_quary+'&type=video&key=AIzaSyCt-7BNhBifrvrNviItFif36rrq8eFWacY')
.then((res)=>{
    return res.json();
}).then((data)=>{
    console.log(data)

    const search_reasult = data.items;
    for (let i = 0; i < search_reasult.length; i++) {
        const singlr_video = search_reasult[i];
        creatsinglrvideoelement(singlr_video);
    }
});
// detail.html
function creatsinglrvideoelement(video){
